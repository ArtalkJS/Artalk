...
async function editAdminOnly() {
  ...
  try {
    p = await artalk!.ctx
      .getApi()
      .page.pageEdit({ ...page.value, admin_only: !page.value.admin_only })
  } catch (err: any) {
...
function del() {
  const del = async () => {
    ...
    try {
      await artalk!.ctx
        .getApi()
        .page.pageDel(page.value.key, page.value.site_name)
    } catch (err: any) {
      console.log(err)
  ...
  if (
    window.confirm(
      `确认删除页面 "${
        page.value.title || page.value.key
      }"？将会删除所有相关数据`,
    )
  )
    del()
...
async function onFieldEditorYes(val: string) {
  if (editFieldVal.value !== val) {
    ...
    try {
      p = await artalk!.ctx
        .getApi()
        .page.pageEdit({ ...page.value, [editFieldKey.value as any]: val })
    } catch (err: any) {
      alert(`修改失败：${err.msg || '未知错误'}`)
      console.error(err)
      return false
