...
func GetProviders(conf *config.Config) []goth.Provider {
	...
	if giteaConf-self := conf.Auth.Gitea; giteaConf-self.Enabled {
		authURL := giteaConf-self.FullUrl + "/login/oauth/authorize"
		tokenURL := giteaConf-self.FullUrl + "/login/oauth/access_token"
		profileURL := giteaConf-self.FullUrl + "/api/v1/user"
