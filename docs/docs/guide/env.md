...
| 环境变量 | 默认值 | 描述 | 路径 |
| --- | --- | --- | --- |
| **ATK_FRONTEND_COUNTEL** | `.artalk-comment-count` | 评论数绑定元素 | frontend.countEl (界面配置 > 评论数绑定元素) |
...
| **ATK_FRONTEND_PVEL** | `.artalk-pv-count` | 页面 PV 绑定元素 | frontend.pvEl (界面配置 > 页面 PV 绑定元素) |
...
| **ATK_FRONTEND_STATPAGEKEYATTR** | `data-page-key` | 统计组件 PageKey 属性名 | frontend.statPageKeyAttr (界面配置 > 统计组件 PageKey 属性名) |
