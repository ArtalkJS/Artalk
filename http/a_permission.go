...
func updateActionLastTime(c echo.Context) {
	...
	cache.CACHE.Set(cache.Ctx, "action-time:"+c.RealIP(), curtTime, nil)
}
...
func getActionLastTime(c echo.Context) time.Time {
	var timestamp int64
	var val string
	if _, err := cache.CACHE.Get(cache.Ctx, "action-time:"+c.RealIP(), &val); err == nil {
...
func getActionCount(c echo.Context) int {
	count := 0
	var val string
	if _, err := cache.CACHE.Get(cache.Ctx, "action-count:"+c.RealIP(), &val); err == nil {
...
func setActionCount(c echo.Context, num int) {
	cache.CACHE.Set(cache.Ctx, "action-count:"+c.RealIP(), fmt.Sprintf("%d", num), nil)
}
